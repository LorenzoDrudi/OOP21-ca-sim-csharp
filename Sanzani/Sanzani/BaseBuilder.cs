...
public class BaseBuilder
{
    ...
    /// <exception cref="InvalidOperationException">Exception thrown when the method that called registerCall was already called.</exception>
    public void RegisterCall()
    {
        var stackFrame = new StackTrace().GetFrames();
        var method = stackFrame[1].GetMethod() ?? throw new NullReferenceException();
        if (this._calledMethods.Contains(method.Name))
        {
            throw new InvalidOperationException(method.Name + " has been called twice.");
        }

        this._calledMethods.Add(method.Name);
    }
    ...
    {
        return this.CheckValue(value, x => x != null, errMsg);
    }
}